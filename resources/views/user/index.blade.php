                                        ...
                                        @if( $users )
                                            @foreach( $users as $user )
                                                ...
                                                <tr id="rowID-{{ $user->id }}">
                                                    ...
                                                    <td>{{$user->name}}</td>                      
                                                    <td>{{$user->email}}</td>
                                                    <td>{!!($user->status == 0) ? '<span class="badge badge-dim disable-check"><i class="fas fa-minus-circle"></i> &nbsp; Block</span>' : '<span class="badge badge-dim  enable-check"> <i class="fas fa-check-circle"></i> </span> Active'!!}</td>
                                                    ...
                                                    <td>
                                                        <div class="btn-group" role="group" aria-label="Basic example">
                                                            @if(in_array('update-user', getUserPermissions()))
                                                                <a class="btn "
                                                                   href='{{ route("users.edit", \Illuminate\Support\Facades\Crypt::encrypt($user->id))  }}'>  <img class ="edits-btn"  src="{{asset('assets/images/svg/ico-edit.svg')}}" > </a>
                                                            @endif
